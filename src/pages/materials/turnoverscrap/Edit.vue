...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>周转材料报废单</div>
    </el-col>
      <el-col :span="4">
...
import {
            requestCreateOrUpdateTurnoverScrapOrder,
            requestGetTurnoverScrapOrderForEdit,
            requestDeleteTurnoverScrapOrderItem,
        }   from 'api/materials/turnoverscraporder';
...
export default {
            ...
            sumRowConf:{
                            ...
                            items:{
                              //合计项配置
                               ...
                               scrapOriginalValue:{text:'报废金额',sumval:0.00,capitalCol:false},
                ...
                get () {
                    return {
                        ...
                        columns: [
                            ...
                            {attr: { prop: 'id', label: '操作', width:65,scopedSlot: 'id',} },
                               ...
                               // {attr: { prop: 'barCode', label: '条码', width:100,  } },   
                                ...
                                {attr: { prop: 'scrapPrice', label: '报废单价', width:100, scopedSlot: 'scrapPrice',  } },   
                                {attr: { prop: 'scrapOriginalValue', label: '报废金额', width:100, scopedSlot: 'scrapOriginalValue',  } },   
                                {attr: { prop: 'scrapReasion', label: '报废原因', width:100, scopedSlot: 'scrapReasion',  } },   
                                //{attr: { prop: 'isRs', label: '', width:100,  } },   
                       ...
                       solSlotConf:[ {  type:'input',   // 类型  目前 支持 input switch datepick
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ...
                                                event:(row,name)=>{  //事件逻辑
                                                    if(this.checkdecimalfn(row.quantity)){
                                                         ...
                                                         row.scrapOriginalValue=parseFloat(row.quantity)*row.scrapPrice;
                                                    }else{
                                                         row.quantity=0.00;
                                        ...
                                        eventconf:{
                                                    ...
                                                    event:(row,name)=>{ 
                                                        if(this.checkdecimalfn(row.scrapPrice)){
                                                            row.scrapOriginalValue=parseFloat(row.scrapPrice)*row.quantity;
                                                        }else{
                                                            row.scrapPrice=0.00;
                                                            this.$message.error('输入类型有误！');
                                                        }
                                    ...
                                    { type:'input',name:'scrapOriginalValue', 
                                        elmentConfig:{
                                                maxlength:1000000, 
                                                minlength:0,
                                                size:"small",
                                        ...
                                        eventconf:{
                                                    ison:true,
                                                    event:(row,name)=>{
                                                         if(this.checkdecimalfn(row.scrapOriginalValue)){
                                                            row.scrapPrice=row.quantity==0?0:parseFloat(row.scrapOriginalValue)/row.quantity;
                                                        }else{
                                                            row.scrapOriginalValue=0.00;
                                                            this.$message.error('输入类型有误！');
                                                        }
                                                        this._sumCount();
                                                    },
             ...
             get () {
                 return {
                     mapconf: [   
                                ...
                                { prop: 'scrapPrice', isplace: false,defaultval:0, selprop:'',  },   
                                { prop: 'scrapOriginalValue', isplace: false,defaultval:0, selprop:'',  },   
                                { prop: 'scrapReasion', isplace: false,defaultval:'', selprop:'',  },   
                                { prop: 'isRs', isplace: true,defaultval:'', selprop:'isRs',  },   
    ...
    methods:{
            _sumCount(colName){
             //计算列的合计值  合计方法计算逻辑
           let bookOriginalValue=0.00,scrapOriginalValue=0.00;
           this.formModel.items.map((item)=>{
               bookOriginalValue=bookOriginalValue+parseFloat(item.bookOriginalValue);
               scrapOriginalValue=scrapOriginalValue+parseFloat(item.scrapOriginalValue);
           })
           this.sumRowConf.items.scrapOriginalValue.sumval=scrapOriginalValue.toFixed(2);
           this.sumRowConf.items.bookOriginalValue.sumval=bookOriginalValue.toFixed(2);
         },
         _formInitial(){
            //初始化表单
            requestGetTurnoverScrapOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteTurnoverScrapOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateTurnoverScrapOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('T_TurnoverScrapOrder',this.formModel.order.orgId,val,(val)=>{
    ...
    components:{
     'yl-selectFromTurnover':selectFromTurnover,
