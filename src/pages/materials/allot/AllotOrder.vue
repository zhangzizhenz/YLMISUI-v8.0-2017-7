<template>
    <yl-panelpage :titleName="types==0?'内部调拨单':'外部调拨单'" :fullscreenisShow="true" :icon="'icon-library'">
        <div slot="content">
            <yl-layout>
                <div slot="fristbox">
                    <yl-toolbar>
...
import {
    requestGetAllotOrderPageList,
    requestDeleteAllotOrder,
    requestGetAllotOrderItemPageList,
    requestAuditAllotOrder,
    requestUnAuditAllotOrder
...
export default {
        ...
        return {
            ...
            funBtnConf: {
                add: {
                    ...
                    permissionSetting: "Materials.Consume.Delivery.AllotOrders.Create"
                },
                edit: {
                    name: 'edit',
                    icon: 'icon-pencil',
                    ...
                    permissionSetting: "Materials.Consume.Delivery.AllotOrders.Edit"
                },
                delete: {
                    name: 'delete',
                    icon: 'icon-trashcan',
                    ...
                    permissionSetting: "Materials.Consume.Delivery.AllotOrders.Delete"
                },
                audit: {
                    name: 'audit',
                    icon: 'icon-user-plus',
                    ...
                    permissionSetting: "Materials.Consume.Delivery.AllotOrders.Audit"
                },
                unAudit: {
                    name: 'unAudit',
                    ...
                    permissionSetting: "Materials.Consume.Delivery.AllotOrders.UnAudit"
                },
            },
            ...
            get() {
                return {
                    ...
                    columns: [
                        ...
                        { attr: { prop: 'orgName', label: '调拨单位', width: 170, } },
                        { attr: { prop: 'orderDate', label: '账期', width: 110, } },
                        //{attr: { prop: 'allotDepId', label: '', width:100,  } },   
                        { attr: { prop: 'allotDepName', label: '收料单位', width: 150, } },
                        { attr: { prop: 'adviceNode', label: '调拨通知号', width: 130, } },
            ...
            get() {
                return {
                    ...
                    columns: [
                        ...
                        { attr: { prop: 'infoName', label: '材料名称', width: 130, } },
                        ...
                        { attr: { prop: 'purchasedSum', label: '调拨金额', width: 100, } },
                        { attr: { prop: 'storeRoom', label: '库别', width: 100, } },
                        { attr: { prop: 'storePosition', label: '存放地', width: 130, } },
                        // {attr: { prop: 'cId', label: '', width:100,  } },   
                        { attr: { prop: 'classNodebh', label: '类别编号', width: 130, } },
                        { attr: { prop: 'manufacturer', label: '生产厂商', width: 130, } },
                        { attr: { prop: 'batchNo', label: '供应方式', width: 130, } },
                        { attr: { prop: 'infoRemark', label: '材料备注', width: 130, } },
                        { attr: { prop: 'managerSum', label: '管理费', width: 100, } },
    ...
    methods: {
       _viewItem(id){
            setTimeout(()=>{
                     this._getAllotOrderItemPageList(id);
            },300)
        },
        //预览单据接口
        _printPage(id) {
            this.reportUrl = config.reportUrl + "/AllotOrder.html?id=" + id;
        ...
        _getAllotOrderPageList() {
            ...
            requestGetAllotOrderPageList(this.mainInput.inputModel).then(data => {
                if (data.success) {
        ...
        _getAllotOrderItemPageList(id) {
            ...
            requestGetAllotOrderItemPageList(this.itemInput.inputModel).then(data => {
                if (data.success) {
        ...
        _delete() {
            this.$confirm('确认要删除所选项目吗?', '提示', {
                ...
                requestDeleteAllotOrder(obj).then(data => {
                    if (data.success) {
                        this._reload();
                        this.$notify({
                            title: '成功',
        ...
        _audit() {
            ...
            let obj = {};
            obj.id = this.selectRows[0].id;
            obj.orgId = this.searchModel.orgId;
            requestAuditAllotOrder(obj).then(data => {
                if (data.success) {
        ...
        _unAudit() {
            ...
            requestUnAuditAllotOrder(obj).then(data => {
                if (data.success) {
                    if (data.result.result > 0) {
                        this.$notify({
                            title: '成功',
