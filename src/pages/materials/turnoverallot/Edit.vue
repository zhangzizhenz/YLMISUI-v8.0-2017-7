...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>周转材料调出单</div>
    </el-col>
      <el-col :span="4">
...
<el-row>
        ...
        <el-col :span="8"> 
         <el-form-item label="收料单位" prop="receiveOrgId">
              <yl-organizeTree
                        v-model="formModel.order.receiveOrgId"
                        :placeholder="'请选择记帐单位'"
                        :isexpand="true" 
...
import {
            requestCreateOrUpdateTurnoverAllotOrder,
            requestGetTurnoverAllotOrderForEdit,
            requestDeleteTurnoverAllotOrderItem,
            requestGetTurnoverInventoryForEdit,
...
export default {
            ...
            sumRowConf:{
                            ...
                            items:{
                              //合计项配置
                               ...
                               allotOriginalValue:{text:'调拨原值',sumval:0.00,capitalCol:false},
                ...
                get () {
                    return {
                        ...
                        columns: [
                            ...
                            {attr: { prop: 'id', label: '操作', width:65,scopedSlot: 'id',} },
                               ...
                               // {attr: { prop: 'storeRoom', label: '库别', width:100,  } },   
                                ...
                                {attr: { prop: 'allotOriginalValue', label: '调拨原值', width:100,  scopedSlot: 'allotOriginalValue', } },   
                        ...
                        solSlotConf:[ {  type:'input',   // 类型  目前 支持 input switch datepick
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ...
                                                event:(row,name)=>{  //事件逻辑
                                                    if(this.checkdecimalfn(row.quantity)){
                                                          ...
                                                          row.allotOriginalValue=parseFloat(row.quantity)*row.allotPrice;
                                                        } else {
                                                                    row.quantity = 0.00;
                                        ...
                                        eventconf:{
                                                    ...
                                                    event:(row,name)=>{ 
                                                        if(this.checkdecimalfn(row.allotPrice)){
                                                            row.allotOriginalValue=parseFloat(row.allotPrice)*row.quantity;
                                                        }else{
                                                            row.allotPrice=0.00;
                                                            this.$message.error('输入类型有误！');
                                                        }
                                    ...
                                    { type:'input',name:'allotOriginalValue', 
                                        elmentConfig:{
                                                maxlength:1000000, 
                                                minlength:0,
                                                size:"small",
                                        ...
                                        eventconf:{
                                                    ison:true,
                                                    event:(row,name)=>{
                                                         if(this.checkdecimalfn(row.allotOriginalValue)){
                                                            row.allotPrice=row.quantity==0?0:parseFloat(row.allotOriginalValue)/row.quantity;
                                                        }else{
                                                            row.allotOriginalValue=0.00;
                                                            this.$message.error('输入类型有误！');
                                                        }
                                                        this._sumCount();
                                                    },
             ...
             get () {
                 return {
                     mapconf: [   
                                ...
                                { prop: 'dataType', isplace: false, defaultval:'82',selprop: '' },  //来源类型  // 0 常用材料库 1 进料登记选材 2 从总需用计划选材  3 进料登记（30进料登记PDA选材 31发料登记选材 32内部调拨登记选材 33外部调拨登记选材 ）4从磅单选材 5 合同选材 6从库存选材     
                                ...
                                { prop: 'allotPrice', isplace: false,defaultval:0, selprop:'',  },   
                                { prop: 'allotOriginalValue', isplace: false,defaultval:0, selprop:'',  },   
    ...
    methods:{
         _sumCount(colName){
             //计算列的合计值  合计方法计算逻辑
           let bookOriginalValue=0.00,allotOriginalValue=0.00;
           this.formModel.items.map((item)=>{
               bookOriginalValue=bookOriginalValue+parseFloat(item.bookOriginalValue);
               allotOriginalValue=allotOriginalValue+parseFloat(item.allotOriginalValue);
           })
           this.sumRowConf.items.allotOriginalValue.sumval=allotOriginalValue.toFixed(2);
           this.sumRowConf.items.bookOriginalValue.sumval=bookOriginalValue.toFixed(2);
         },
         _formInitial(){
            //初始化表单
            requestGetTurnoverAllotOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteTurnoverAllotOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateTurnoverAllotOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('T_TurnoverAllotOrder',this.formModel.order.orgId,val,(val)=>{
