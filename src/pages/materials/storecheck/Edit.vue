...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>原材料盘点</div>
    </el-col>
      <el-col :span="4">
...
import {
            requestCreateOrUpdateStoreCheckOrder,
            requestGetStoreCheckOrderForEdit,
            requestDeleteStoreCheckOrderItemItem,
...
export default {
                ...
                get () {
                    return {
                        ...
                        columns: [
                            ...
                            {attr: { prop: 'infoModel', label: '规格型号', width:140,  } }, 
                             {attr: { prop: 'infoUnit', label: '单位', width:100,  } },
                            {attr: { prop: 'quantity', label: '盘点数量', width:120,  scopedSlot: 'quantity', } },   
    ...
    methods:{
        ...
        //    })
         ...
         _formInitial(){
            //初始化表单
            requestGetStoreCheckOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteStoreCheckOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateStoreCheckOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('M_StoreCheckOrder',this.formModel.order.orgId,val,(val)=>{
