...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>周转材料收料单</div>
    </el-col>
      <el-col :span="4">
        ...
        <el-col :span="8"> 
         ...
         <el-form-item label="存放地" prop="storePosition">
             <yl-comDataDictionary :code="'StorePostion'" 
                            v-model="formModel.order.storePosition" 
                            :placeholder="'请选择'"
                            style="width:80%">
        ...
        <el-col :span="8"> 
         <el-form-item label="库别" prop="storeRoom">
             <yl-dataDictionaryForTree 
                    :code="'StoreRoom'"
                    placeholder="请选择库别" 
                    v-model="formModel.order.storeRoom"
                    :isexpand="true"
                    @getCurrentNode="_getCurrentStroreRoom"
                    style="width:80%">
              </yl-dataDictionaryForTree>
         </el-form-item>
       </el-col>
...
import {
            requestCreateOrUpdateTurnoverReceiveOrder,
            requestGetTurnoverReceiveOrderForEdit,
            requestDeleteTurnoverReceiveOrderItem,
...
export default {
                ...
                get () {
                    return {
                        ...
                        columns: [
                                ...
                                {attr: { prop: 'amortizeType', label: '摊销方式', width:100,  scopedSlot: 'amortizeType',  } },  
                                ...
                                {attr: { prop: 'purchasedPrice', label: '税后单价', width:100,scopedSlot: 'purchasedPrice',  } },   
                                ...
                                {attr: { prop: 'bookSum', label: '原值', width:100, scopedSlot: 'bookSum', } },   
                                //{attr: { prop: 'isRule', label: '', width:100,  } },   
                                {attr: { prop: 'remark', label: '备注', width:150, scopedSlot: 'remark', } },  
                        ...
                        solSlotConf:[ {  type:'input',   // 类型  目前 支持 input switch datepick
                                    ...
                                    {  type:'input',   // 类型  目前 支持 input switch datepick
                                     ...
                                     { type:'input',name:'purchasedPrice', 
                                        elmentConfig:{
                                                maxlength:1000000, 
                                                minlength:0,
                                                size:"small",
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ...
                                                event:(row,name)=>{  //事件逻辑
                                                 if(this.checkdecimalfn(row.purchasedSum)){
                                                         row.purchasedPrice=(row.quantity==0?0:parseFloat(row.purchasedSum)/row.quantity).toFixed(4);
                                                         row.unitPrice=(row.purchasedPrice/(1+row.taxRate/100)).toFixed(4);
                                                         row.bookPrice=row.unitPrice;
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ...
                                                event:(row,name)=>{  //事件逻辑
                                                      ...
                                                      if(this.checkdecimalfn(row.taxSum)){
                                                         if(row.quantity!=0){
                                                         row.purchasedPrice=(row.purchasedSum/row.quantity).toFixed(4);
                                                         //row.bookPrice=(parseFloat(row.unitPrice)).toFixed(4);
                                                         }else{
                                                            row.purchasedPrice=0;
                                                            row.bookPrice=0;
                                                         }
             ...
             get () {
                 return {
                     mapconf: [   
                                ...
                                { prop: 'purchasedPrice', isplace: false,defaultval:0, selprop:'',  },   
    ...
    methods:{
         ...
         _formInitial(){
            //初始化表单
            requestGetTurnoverReceiveOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteTurnoverReceiveOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateTurnoverReceiveOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('T_TurnoverReceiveOrder',this.formModel.order.orgId,val,(val)=>{
