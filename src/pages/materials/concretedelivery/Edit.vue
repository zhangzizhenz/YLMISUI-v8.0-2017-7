...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>混凝土出料</div>
    </el-col>
      <el-col :span="4">
...
<el-row>             
        ...
        <el-col :span="8"> 
         <el-form-item label="运输单号" prop="transportNum">
             <el-input v-model="formModel.order.transportNum" :placeholder="'请输入运输单号'" style="width:80%" ></el-input>
         </el-form-item>
       </el-col>
</el-row>
...
import {
            requestCreateOrUpdateConcreteDeliveryOrder,
            requestGetConcreteDeliveryOrderForEdit,
            requestDeleteConcreteDeliveryOrderItemItem,
...
export default {
                    ...
                    return {
                        ...
                        columns: [
                            ...
                            //列配合
                                ...
                                {attr: { prop: 'infoName', label: '材料名称', width:160,  } },   
                                {attr: { prop: 'infoModel', label: '规格型号', width:160,  } },   
                                {attr: { prop: 'infoUnit', label: '单位', width:100,  } }, 
                                {attr: { prop: 'quantity', label: '出料数量', width:100, scopedSlot: 'quantity', } }, 
                                 {attr: { prop: 'remark', label: '备注', scopedSlot: 'remark', } },     
                                //{attr: { prop: 'barCode', label: '条码', width:100,  } },   
                                //{attr: { prop: 'storeRoom', label: '库别', width:100,  } },   
                                //{attr: { prop: 'storePosition', label: '存放地',  } },   
    ...
    methods:{
        ...
        //    })
         ...
         _formInitial(){
            //初始化表单
            requestGetConcreteDeliveryOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteConcreteDeliveryOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateConcreteDeliveryOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('M_ConcreteDeliveryOrder',this.formModel.order.orgId,val,(val)=>{
