...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span> 用料单位月应耗量</div>
    </el-col>
      <el-col :span="4">
...
import {
            requestCreateOrUpdateLabourMonthConsumptionOrder,
            requestGetLabourMonthConsumptionOrderForEdit,
            requestDeleteLabourMonthConsumptionOrderItem,
...
export default {
    data(){
        return{
            ...
            rules: {
                            ...
                            labourName: [
                              { required: true, message: '用料单位名称不能为空！', trigger: 'change' }
                            ],
            },
            sumRowConf:{
                            disable:false, //是否显示
             ...
             get () {
                 return {
                     mapconf: [   
                                ...
                                { prop: 'lossRate', isplace: false,defaultval:0, selprop:'lossRate',  },   
                                { prop: 'lossQuantity', isplace: false,defaultval:0, selprop:'lossQuantity',  },   
    ...
    methods:{
        _sumCount(colName) {
         ...
         _formInitial(){
            //初始化表单
            requestGetLabourMonthConsumptionOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteLabourMonthConsumptionOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateLabourMonthConsumptionOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
        ...
        _getCode(val){
             this.generateOrderCode('P_LabourMonthConsumptionOrder',this.formModel.order.orgId,val,(val)=>{
