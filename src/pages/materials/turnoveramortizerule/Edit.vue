...
<div>
  <el-row class="yl-order-header">
    ...
    <el-col :span="18" > 
     <div class="yl-order-title"><span class="yl-order-small-title">{{title}}&nbsp;&nbsp;&nbsp;</span>摊销规则</div>
    </el-col>
      <el-col :span="4">
  ...
  <el-form  :model="formModel.order"  :rules="rules" ref="formModel" label-width="100px" >
   ...
   <el-form-item style="text-align:right;" >
      <el-button type="primary" @click="_onReceiveSelect"><i class="icon-plus"></i>收料选材</el-button>
      <el-button type="primary" @click="_onAllotInSelect"><i class="icon-plus"></i>调入选材</el-button>
 ...
 <el-dialog ref="selectDialog3" title="选择"  
                 v-model="addReceiveFormVisible" size="large"  
                 top="8%" 
                 :modal="false"
                 :lock-scroll="true"
               ...
               <yl-selectReceiveFromComMat    
                    :isSelectClose="true"
                    @close="_reclose"
                    v-if="addReceiveFormVisible">
                </yl-selectReceiveFromComMat>
  </el-dialog>
<el-dialog ref="selectDialog4" title="选择"  
                 v-model="addAllotInFormVisible" size="large"  
                 top="8%" 
                 :modal="false"
                 :lock-scroll="true"
                 :close-on-press-escape="false"
                 :modal-append-to-body="false" 
                 :close-on-click-modal="false"  >
               <yl-selectAllotInFromComMat    
                    :isSelectClose="true"
                    @close="_reclose"
                    v-if="addAllotInFormVisible">
                </yl-selectAllotInFromComMat>
  </el-dialog>
</div>
</template>
...
import selectReceiveFromComMat from 'ocomponents/selectmodule/selectFromTReceive'
import selectAllotInFromComMat from 'ocomponents/selectmodule/selectFromTAllotIn'
import config from 'static/js/config';
import {
            requestCreateOrUpdateTurnoverAmortizeRuleOrder,
            requestGetTurnoverAmortizeRuleOrderForEdit,
            requestDeleteTurnoverAmortizeRuleOrderItem,
        }   from 'api/materials/turnoveramortizerule';
...
export default {
                    ...
                    return {
                        ...
                        columns: [
                            ...
                            //列配合  
                                ...
                                {attr: { prop: 'residualValueRate', label: '残值率', width:100,   scopedSlot: 'residualValueRate',} },   
                                {attr: { prop: 'residualValue', label: '残值', width:100,  scopedSlot: 'residualValue', } },   
                                {attr: { prop: 'amortizeUnitPrice', label: '摊销单价', width:100,} },   
                                {attr: { prop: 'amortizeSum', label: '需摊销值', width:100,  } },   
                                {attr: { prop: 'amortizeTime', label: '总工期/工作量', width:120,   scopedSlot: 'amortizeTime',} },
                        ...
                        solSlotConf:[
                                        ...
                                        elmentConfig:{  //控件配置   参照elementui配置参数
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ...
                                                event:(row,name)=>{  //事件逻辑
                                                    console.log(row);
                                                    if(this.checkdecimalfn(row.residualValueRate)){
                                                         row.residualValue=parseFloat(row.residualValueRate)*0.01*row.bookOriginalValue;
                                                         row.amortizeSum=row.bookOriginalValue-row.residualValue;
                                                         row.amortizeUnitPrice=row.quantity==0?0:(row.amortizeTime==0?0:row.amortizeSum/row.quantity/row.amortizeTime);
                                                    }else{
                                                         row.residualValueRate=0.00;
                                        ...
                                        elmentConfig:{  //控件配置   参照elementui配置参数
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ison:true,  //开启事件
                                                event:(row,name)=>{  //事件逻辑
                                                   if(this.checkdecimalfn(row.residualValue)){
                                                         row.residualValueRate=row.bookOriginalValue==0?0:parseFloat(row.residualValue)/row.bookOriginalValue*100;
                                                         row.amortizeSum=row.bookOriginalValue-row.residualValue;
                                                         row.amortizeUnitPrice=row.quantity==0?0:(row.amortizeTime==0?0:row.amortizeSum/row.quantity/row.amortizeTime);
                                                    }else{
                                                         row.residualValue=0.00;
                                                         this.$message.error('输入类型有误！');
                                                    }
                                                    this._sumCount();
                                                },
                                        ...
                                        elmentConfig:{  //控件配置   参照elementui配置参数
                                         ...
                                         eventconf:{     //事件配置  目前只支持input的 blur事件
                                                ison:true,  //开启事件
                                                event:(row,name)=>{  //事件逻辑
                                                    if(this.checkdecimalfn(row.amortizeTime)){
                                                         row.amortizeUnitPrice=row.quantity==0?0:(row.amortizeTime==0?0:(row.bookOriginalValue-row.residualValue)/row.quantity/row.amortizeTime);
                                                    }else{
                                                         row.amortizeTime=0.00;
                                                         this.$message.error('输入类型有误！');
                                                    }
                                                    this._sumCount();
                                                },
             ...
             get () {
                 return {
                     mapconf: [   
                                ...
                                { prop: 'receiveItemNM', isplace: true,defaultval:'', selprop:'id',  },   
                                { prop: 'receiveTypeId', isplace: true,defaultval:'', selprop:'receiveTypeId',  },   
                                { prop: 'storeMonth', isplace: true,defaultval:'', selprop:'storeMonth',  },   
                                ...
                                { prop: 'bookOriginalValue', isplace: true,defaultval:'', selprop:'bookSum',  },   
                                { prop: 'residualValueRate', isplace: false,defaultval:0, selprop:'residualValueRate',  },   
                                { prop: 'residualValue', isplace: false,defaultval:0, selprop:'residualValue',  },   
                                { prop: 'amortizeUnitPrice', isplace: false,defaultval:0, selprop:'amortizeUnitPrice',  },   
                                { prop: 'amortizeSum', isplace: false,defaultval:0, selprop:'amortizeSum',  },   
                                { prop: 'amortizeTime', isplace: false,defaultval:0, selprop:'amortizeTime',  },   
                                // { prop: 'dateType', isplace: true,defaultval:'', selprop:'dateType',  },   
    ...
    methods:{
         _sumCount(colName){
             //计算列的合计值  合计方法计算逻辑
           let bookOriginalValueT=0.00;
            this.formModel.items.map((item)=>{
                //凡是手动输入后的input都会转化为str  因此计算时要转化一下。
               bookOriginalValueT=bookOriginalValueT+parseFloat(item.bookOriginalValue);
           })
           this.sumRowConf.items.bookOriginalValue.sumval=bookOriginalValueT.toFixed(2);
         },
         _formInitial(){
            //初始化表单
            requestGetTurnoverAmortizeRuleOrderForEdit(this.selectRow.id).then(data=>{
                if(data.success){
         ...
         _delItem(row){
                ...
                modal:false
                        ...
                        if(row.id!=="0"){
                            ...
                            let obj={};
                            obj.id=row.id;
                            requestDeleteTurnoverAmortizeRuleOrderItem(obj).then(data=>{
                                if(data.success){
         ...
         _onSubmit() {
                ...
                this.$refs.formModel.validate((valid) => {
                    if (valid) {
                        ...
                        this.loading=true;
                        requestCreateOrUpdateTurnoverAmortizeRuleOrder(this.formModel).then((data)=>{
                            if(data.success){
                                this.$notify({
                                    title: '成功',
                                    message: '保存数据成功！',
         ...
         _complete(){
          ...
          _reclose(){
                ...
                this.addAllotInFormVisible=false;
                this._sumCount();
          },
          _onReceiveSelect(){
             //打开选材框的权限判断tableEditMapConfig
             this.maps=this.tableEditMapConfig;
             this.addReceiveFormVisible=true;
          },
          _onAllotInSelect(){
             this.maps=this.tableEditMapConfig1;
             this.addAllotInFormVisible=true;
          },
        ...
        _getCode(val){
             this.generateOrderCode('T_TurnoverAmortizeRuleOrder',this.formModel.order.orgId,val,(val)=>{
    ...
    components:{
       ...
       'yl-selectReceiveFromComMat':selectReceiveFromComMat,
       'yl-selectAllotInFromComMat':selectAllotInFromComMat
    },
    watch:{ 
