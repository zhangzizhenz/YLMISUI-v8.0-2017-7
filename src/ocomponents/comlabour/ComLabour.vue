...
vue.component('my-item-labour', {
    functional: true,
    render: function (h, ctx) {
      var item = ctx.props.item;
      return h('li', ctx.data, [
        h('div', { attrs: { class: 'name' } }, [item.labourName]),
        h('div', { attrs: { class: 'addr' } }, [item.labourContact]),
        h('div', { attrs: { class: 'addr' } }, [item.labourPhone])
      ]);
    },
    props: {
...
export default {
  data() {
    ...
    methods: {
      loadAll() {
            var inputArr=[ 
                            {key:"LabourName",op:"LIKE",value:this.value},
                            {key:"OrgId",op:"EQ",value:this.manageOrgId},
                         ];
            //排序
            this.mainInput.inputModel.sorting="LabourName asc";
      ...
      createFilter(queryString) {
          return (state) => {  
                 return (state.labourName.indexOf(queryString.toLowerCase()) === 0);
          };
      },
      _select(item) {
        //选择事件
        this.$emit('selectNode',item);
        this.$emit('input',item.labourName);
      },
        _clearInput(){
         this.$emit('input',"");
         this.$emit('clearBack');
